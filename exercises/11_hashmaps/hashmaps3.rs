...
fn build_scores_table(results: &str) -> HashMap<&str, TeamScores> {
    ...
    for line in results.lines() {
        ...
        if scores.contains_key(team_1_name) {
            let original = scores.get(team_1_name).unwrap();
            scores.insert(team_1_name, TeamScores {
                 goals_scored: original.goals_scored + team_1_score,
                 goals_conceded: original.goals_conceded + team_2_score });
        } else {
        ...
        if scores.contains_key(team_2_name) {
            let original = scores.get(team_2_name).unwrap();
            scores.insert(team_2_name, TeamScores {
                 goals_scored: original.goals_scored + team_2_score,
                 goals_conceded: original.goals_conceded + team_1_score });
        } else {
